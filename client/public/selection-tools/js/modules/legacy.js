...
(function() {
	...
	(function() {
		...
		script.onerror = function() {
			...
			// 提示用户检查文件
			alert('⚠️ XLSX库加载失败\n\n请确认 libs/xlsx.full.min.js 文件存在。');
		};
		document.head.appendChild(script);
	})();
	...
	// M列（12列）1-3行各自横向合并（空着）
	// 留空
	
	// N列（13列）1-3行各自横向合并（空着）
	// 留空
...
console.log('⚠️ B4/K4单元格换行设置已跳过，避免合并单元格错误');
	...
	window.refreshSerialNumbers = function(dataStartRow = 5, footerStartRow = window.currentFooterStartRow || 19) {
		try {
			...
			// 找到"安装费"所在行作为表尾起始，如果找不到则使用传入的footerStartRow
			let actualFooterStart = footerStartRow;
			for (let r = dataStartRow; r < sheetData.length; r++) {
				const row = sheetData[r];
				if (row && row[2]) {
					const cellValue = typeof row[2] === 'object' && 'v' in row[2] ? row[2].v : row[2];
		...
		// 表尾固定5行，序号只到第4行（安装费/钢材用量/电器材料/电线电缆）
		// 第5行是合计，不编号
		for (let i = 0; i < 4; i++) {
		...
		if (totalRow < sheetData.length) {
			...
			console.log(`✅ 序号已更新 - 数据区: ${dataStartRow+1}-${actualFooterStart}行, 表尾: ${actualFooterStart+1}-${actualFooterStart+4}行, 合计: ${actualFooterStart+5}行`);
		} catch(e) {
	...
	function initDeviceListLogic() {
		...
		const updateDeviceListFromTable = function() {
			try {
				...
				// 获取当前表格数据 - 使用getAllSheets获取当前工作表数据
				let currentSheetData;
				try {
					// 尝试多种方式获取数据
					if (luckysheet.getSheetData) {
						currentSheetData = luckysheet.getSheetData();
					} else if (luckysheet.getAllSheets && luckysheet.getAllSheets().length > 0) {
						const currentSheet = luckysheet.getAllSheets()[0];
						currentSheetData = currentSheet.celldata || [];
						// 转换celldata格式为二维数组
...
if (mStr !== '/') {
	...
	if (luckysheet.refresh) {
		...
		// 设置表头内容（后移到新位置）
		// ABCD列（0-3列）1-3行合并显示公司名称
			data[0][0] = '温岭市泽国化工机械有限公司';
		...
		function handleFileSelect(event) {
			...
			LuckyExcel.transformExcelToLucky(file, function(exportJson, luckysheetfile){
				...
				if (versionInfo.isSimplified && versionInfo.detectedColumns > 14) {
					...
					setTimeout(() => {
						try {
							luckysheet.setCellValue(0, 0, '温岭市泽国化工机械有限公司');
...
async function saveToExcel() {
	try {
		...
		const defaultFileName = `HJSJ${dateStr}QDZX004V1.0-${Math.round(granulatorCount)}台项目（设备参数选型）.xlsx`;
		console.log(`📄 生成文件名: ${defaultFileName}`);
...
window.selectSheetsForExport = function(allSheets) {
	...
	function setStyleAndValue(data, worksheet, table) {
        ...
        for (let r = 0; r < effectiveMaxRow; r++) {
            ...
            // ✅ 获取实际显示值（带本地回退，避免 window.getCellText 未定义）
                    let actualValue = (typeof window.getCellText === 'function') ? window.getCellText(cellData) : (function cdToText(cd){
                        if (cd === undefined || cd === null) return '';
                        if (typeof cd === 'string' || typeof cd === 'number') return String(cd).trim();
                        if (typeof cd === 'object') {
			...
			if (r === 3 || r === 4) {
				if (r === 4) {
					...
					// 按原表头保存
					if (actualValue) cell.value = actualValue;
				}
			}
					// A列（索引0）序号强制为数字格式
				...
				// H列（索引7）：简化版→数据行为 F*G，合计行SUM；完整版按值
				else if (c === 7) {
					if (isSimplified) {
						if (isTotalRow) {
			...
			// J列（索引9）：简化版合计行为"/"，数据行保存IF；完整版保持现有
			else if (c === 9) {
				...
				if (isTotalRow) {
                    ...
                    console.log(`📐 J列保存IFERROR(IF(I*H=0,"",I*H),"") 公式: ${cell.address}`);
				}
				// 非数据行且非合计行，不保存
			}
				// K列（索引10）：简化版合计SUM；数据行保存IF；完整版维持原有
				else if (c === 10) {
   ...
   // M列（索引12）：简化版数据行为 F×L，合计行SUM；完整版为 L×H
		...
		if (isTotalRow) {
           ...
           // 数据行：简化版 M=F×L；完整版 M=L×H
           if (isSimplified) {
               cell.value = { formula: `IFERROR(IF(F${excelRow}*L${excelRow}=0,"",F${excelRow}*L${excelRow}),"")` };
               console.log(`📐 M列保存IFERROR(IF(F*L=0,"",F*L),"") 公式: ${cell.address} (简化版)`);
           } else {
		...
		// D列（索引3）、E列（索引4）、B列第4行（设备位号）、K列第4行（电机功率(KW)）需要自动换行
				const isColumnD = (c === 3);
				const isColumnE = (c === 4);
			const isDevicePositionHeader = (r === 3 && c === 1); // B4: 设备位号
			const isMotorPowerHeader = (r === 3 && c === 10); // K4: 电机功率(KW)
			// 公司名换行
			const actualTextForStyle = (typeof actualValue !== 'undefined' && actualValue !== null) ? String(actualValue) : '';
			const isCompanyNameCell = actualTextForStyle.indexOf('温岭市泽国化工机械有限公司') !== -1;
	...
	console.log(`📊 当前工作表: ${currentSheet ? currentSheet.name : '未知'}，${isSimplified ? '简化版' : '完整版'}`);
        ...
        // 在指定位置插入一行前，先显式选中该行，避免API内部读取 row[1] 报错
			...
			if (isSimplified) {
				...
				// K列：总设备功率（直接计算值）
				const kInitialValue = jInitialValue ? (parseFloat(jInitialValue) * quantity) : '';
				luckysheet.setCellValue(insertRow, 10, {
					v: kInitialValue || '',
					m: kInitialValue ? String(kInitialValue) : '',
					ct: { fa: "General", t: "n" },
					ff: "SimSun",
					fs: 10,
					ht: 0,
...
// ⚠️ 立即计算公式列的值并显示（只设置值，不设置公式，避免循环引用）
			...
			if (isSimplified) {
				...
				if (!isExpression && iStr && iStr !== '/') {
					...
					if (jResult > 0) {
						...
						// K列：J*F（总设备功率 = 单台设备功率 * 数量）
						const kResult = jResult * fNum;
						if (kResult > 0) {
							luckysheet.setCellValue(insertRow, 10, {
								v: kResult,
								m: String(kResult),
							...
							console.log(`✅ 简化版K列已计算: ${kResult}`);
						}
					}
				}
				
				// M列：L*F（设备总价 = 设备单价 * 数量）
				const lVal = sheetData[insertRow] && sheetData[insertRow][11];  // L列：单价
				const lNum = parseFloat((lVal && typeof lVal === 'object') ? (lVal.v || lVal.m) : lVal) || 0;
				const mResult = lNum * fNum;
				...
				if (mResult > 0) {
					...
					console.log(`✅ 简化版M列已计算: ${mResult.toFixed(2)}`);
				}
	...
	let pdfConfig = {
		...
		watermarkOpacity: 0.05,  // 降低透明度：0.1 → 0.05
		watermarkDensity: 1,
		watermarkColor: 'gray'
	};
	...
	function updatePdfConfig() {
		...
		pdfConfig.watermarkFontSize = parseInt(document.getElementById('watermarkFontSize').value) || 80;
		pdfConfig.watermarkRotation = parseInt(document.getElementById('watermarkRotation').value) || -45;
		pdfConfig.watermarkOpacity = parseFloat(document.getElementById('watermarkOpacity').value) || 0.1;
		pdfConfig.watermarkDensity = parseInt(document.getElementById('watermarkDensity').value) || 1;
...
function generatePreview() {
	...
	console.log('📏 PDF导出使用的列宽配置:', columnlenConfig);
        ...
        // 生成colgroup来定义列宽（统一线条宽度为1px，更清晰）
    ...
    let cellValue = '';
    let cellStyle = 'border: 1px solid #333; padding: 4px; line-height: 1.3; '; // 边框1px，padding增加到4px
	let rowspan = 1;
	let colspan = 1;
...
function exportToPdf() {
	...
	function renderPage() {
		...
		html2canvas(pageDiv, {
			...
			const imgData = canvas.toDataURL('image/jpeg', 0.95);  // 使用JPEG格式减小文件，质量95%
			...
			// 计算图片尺寸，适应页面（减少边距，增加内容空间）
            const imgWidth = pageWidth - 10; // 左右各留5mm
            const imgHeight = (canvas.height * imgWidth) / canvas.width;
			
            // 计算实际可用高度
            const maxImgHeight = pageHeight - 15; // 上下各留约7.5mm
...
// 保存PDF文件
function savePdfFile(pdf, confirmBtn) {
