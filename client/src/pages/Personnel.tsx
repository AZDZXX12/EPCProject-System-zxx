    ...
    personnel.forEach((person) => {
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        data.push({
          ...
          overtime,
          workHours:
            status === 'present' || status === 'late' ? 8 : status === 'overtime' ? overtime : 0,
        });
      }
    });
  ...
  const handleExportAttendance = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        `${selectedMonth.format('YYYY年MM月')}考勤`
      );
    ...
    try {
      ...
      personnel.forEach((person) => {
        ...
        summary.push({
          ...
          出勤率:
            ((totalWorkDays / (totalWorkDays + absentDays + leaveDays)) * 100).toFixed(1) + '%',
        });
      });
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: '#666', margin: 0 }}>化工设备安装工程施工人员信息与考勤管理</p>
      </div>
              ...
              <Card title={<span style={{ fontSize: 16, fontWeight: 600 }}>今日考勤记录</span>}>
                <Timeline
                  items={attendanceLogs.map((log, index) => ({
                    key: index,
                    color:
                      log.type === 'checkin' ? 'green' : log.type === 'checkout' ? 'red' : 'blue',
                    children: (
                      <>
                ...
                <Table
                  columns={[
                    ...
                    },
                    {
                      ...
                      render: (hours: number) => (
                        <span
                          style={{ fontWeight: 600, color: hours >= 8 ? '#52c41a' : '#faad14' }}
                        >
                          {hours}h
                        </span>
                      ),
