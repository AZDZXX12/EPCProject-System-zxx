...
import { Card, Button, Row, Col, Space, Tag, Progress, List, Typography, Badge, Modal, Form, Input, Select, Statistic, Table, message } from 'antd';
import { 
  PlusOutlined,
  ProjectOutlined,
  CheckCircleOutlined,
...
const Workspace: React.FC = () => {
  ...
  const handleSaveProject = async (values: any) => {
    try {
      if (editingProject) {
        ...
        const response = await fetch(`${API_ENDPOINTS.projects}/${editingProject.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  // 通知信息
  const notifications = [
    { id: 1, type: 'warning', title: '项目进度提醒', message: '化工设备生产线项目进度45%，需加快施工进度', time: '2小时前', projectId: 'PROJ-001' },
    { id: 2, type: 'info', title: '新项目立项', message: '石油炼化装置改造项目已完成立项审批', time: '5小时前', projectId: 'PROJ-002' },
    { id: 3, type: 'success', title: '里程碑完成', message: '设备采购阶段已完成验收', time: '1天前', projectId: 'PROJ-001' }
  ];

  // 待办事项
  const todoItems = [
    { id: 1, title: '审批施工进度规划', project: '化工设备生产线', priority: 'high', deadline: '今天' },
    { id: 2, title: '确认设备到货时间', project: '石油炼化装置', priority: 'medium', deadline: '明天' },
        ...
        {currentProject && (
          <Card 
            ...
            extra={
              <Button 
                type="primary" 
                onClick={() => navigate('/construction-management')}
              >
                进入施工管理
              </Button>
            }
          ...
          >
            <Row gutter={16}>
              <Col span={18}>
                <Title level={4} style={{ marginTop: 0 }}>{currentProject.name}</Title>
                ...
                <div style={{ marginTop: 16 }}>
                  <Space size="large">
                    ...
                    <Space direction="vertical" size={0}>
                      ...
                      <Text strong><CalendarOutlined /> {currentProject.start_date}</Text>
                    </Space>
                    <Space direction="vertical" size={0}>
                      <Text type="secondary" style={{ fontSize: 12 }}>结束日期</Text>
                      <Text strong><CalendarOutlined /> {currentProject.end_date}</Text>
                    </Space>
                    <Space direction="vertical" size={0}>
                      <Text type="secondary" style={{ fontSize: 12 }}>项目状态</Text>
            ...
            <List
                ...
                renderItem={(item: any) => (
                  ...
                  <List.Item.Meta
                      ...
                      description={<Text type="secondary" style={{ fontSize: 12 }}>{item.project}</Text>}
                  />
                </List.Item>
              )}
            />
              ...
              <Space direction="vertical" style={{ width: '100%' }} size="middle">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Space>
                    ...
                    <div>
                      <Text strong style={{ display: 'block', fontSize: 13 }}>张工程师</Text>
                      <Text type="secondary" style={{ fontSize: 11 }}>项目经理</Text>
                    </div>
                  </Space>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Space>
                    ...
                    <div>
                      <Text strong style={{ display: 'block', fontSize: 13 }}>李工</Text>
                      <Text type="secondary" style={{ fontSize: 11 }}>施工负责人</Text>
                    </div>
                  </Space>
                  <Tag color="success">在线</Tag>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Space>
                    ...
                    <div>
                      <Text strong style={{ display: 'block', fontSize: 13 }}>王主管</Text>
                      <Text type="secondary" style={{ fontSize: 11 }}>质检主管</Text>
                    </div>
                  </Space>
